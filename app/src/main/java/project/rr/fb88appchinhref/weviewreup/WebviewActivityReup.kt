package project.rr.fb88appchinhref.weviewreup
...
import project.rr.fb88appchinhref.common.DbFirebase
import project.rr.fb88appchinhref.common.presentation.dashboard.MainActivity
import project.rr.fb88appchinhref.databinding.ActivityWebviewReupBinding

class WebviewActivityReup : AppCompatActivity() {


    private lateinit var binding: ActivityWebviewReupBinding


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        binding = ActivityWebviewReupBinding.inflate(layoutInflater)
...
private fun init() {
    ...
    with(webseting) {
        this@WebviewActivityReup.getDir(
            "cache", MODE_PRIVATE
        )?.path
        domStorageEnabled = true
    ...
    binding.firstWeb.webChromeClient = object : WebChromeClient() {
        ...
        override fun onCreateWindow(
            ...
            val newWebView = this@WebviewActivityReup?.let { WebView(it) }
                ...
                override fun shouldOverrideUrlLoading(view: WebView, url: String): Boolean {
                    ...
                    if (url.startsWith("http") || url.startsWith("https")) {
                        ...
                        try {
                            ...
                            this@WebviewActivityReup?.startActivity(`in`)

                        } catch (ex: ActivityNotFoundException) {
                            val makeShortText = "The Application has not been installed"
                            Toast.makeText(
                                this@WebviewActivityReup,
                                makeShortText,
                                Toast.LENGTH_SHORT
                            ).show()
                        }
    ...
    binding.firstWeb.webViewClient = object : WebViewClient() {
        ...
        override fun onReceivedSslError(
            ...
            val builder = android.app.AlertDialog.Builder(this@WebviewActivityReup)
        ...
        override fun shouldOverrideUrlLoading(view: WebView, url: String): Boolean {
            if (url.startsWith("http") || url.startsWith("https")) {
                ...
                this@WebviewActivityReup?.startActivity(newInt)
            } else {
                try {
                    val `in` = Intent(Intent.ACTION_VIEW, Uri.parse(url))
                    this@WebviewActivityReup.startActivity(`in`)
                    ...
                    val makeShortText = "The Application has not been installed"
                    Toast.makeText(this@WebviewActivityReup, makeShortText, Toast.LENGTH_SHORT)
                        .show()
                }
            }
            return true
    ...
    companion object{
        fun getStartIntent (context: Context) = Intent(context, WebviewActivityReup::class.java)
    }
